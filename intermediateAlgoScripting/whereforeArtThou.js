...
function whatIsInAName(collection, source) {
    ...
    for (var i = 0; i < collection.length; i++) {
        for (var j = 0; j < sprop.length; j++) {
            // test if args have same property
            if (collection[i].hasOwnProperty(sprop[j])) {
                var prp = sprop[j];
                // test to see if property has same value
                if (collection[i][prp] == source[prp])
                    isIn = true;
            } else {
                isIn = false;
            }
...
console.log(whatIsInAName([{ "a": 1, "b": 2 }, { "a": 1 },
    { "a": 1, "b": 2, "c": 2 }], { "a": 1, "c": 2 }));
// [{ "a": 1, "b": 2, "c": 2 }]
